...
Partial support:
  ...
  - This doesn't currently support .include directives fully (the included file
    will not be transformed, so ACC instructions won't work there).
...
def parse_positionals(
    ...
    if '-h' in others or '--help' in others:
        print('acc_as.py:\n\n'
              ...
              '\n'
              '  --acc-translate: Translate the input and dump to '
...
def _unpack_lx(where: str, mnemonic: str,
    ...
    if set(op_to_expr.keys()) != {'grd', 'imm'}:
        ...
        raise RuntimeError(f'When expanding {umnem}, got wrong op_to_expr '
                           ...
                           f'expand_{mnemonic} in acc_as.py and the operands '
...
def expand_la(where: str, op_to_expr: Dict[str, Optional[str]]) -> List[str]:
    ...
    #
    # For ACC, both IMEM and DMEM are small. This means that we can represent
...
class Transformer:
    '''A simple parser/transformer for ACC input files
...
def main(argv: List[str]) -> int:
    ...
    files = files or ['--']
    just_translate = '--acc-translate' in flags
    ...
    with tempfile.TemporaryDirectory(suffix='.acc-gcc') as tmpdir:
        try:
            ...
            if insn.python_pseudo_op:
                if insn.mnemonic not in _PSEUDO_OP_ASSEMBLERS:
                    sys.stderr.write(
                        ...
                        "but acc_as.py doesn't have a custom assembler "
                        "for it.\n".format(insn.mnemonic))
                    return 1

        # Try to match up ACC instruction encodings with .insn schemes (as stored
        # in RISCV_FORMATS).
        mnem_to_rve = find_insn_schemes(insns_file.mnemonic_to_insn)

        with tempfile.TemporaryDirectory(suffix='.acc-as') as tmpdir:
            try:
