...
package acc_pkg;
  ...
  // Size of DMEM scratch area. The total DMEM size is ACC_DMEM_SIZE + DmemScratchSizeByte. Note
  // that some of the Python tooling depends on this parameter (it needs to know the full DMEM size,
  // but regtool only gives it ACC_DMEM_SIZE). If changing this, you'll also need to edit
  ...
  // Command to execute. See the CMD register description in acc.hjson for details.
  ...
  // Status register values. See the STATUS register description in acc.hjson for details.
  typedef enum logic [7:0] {
    StatusIdle            = 8'h00,
  ...
  // The error signals that are generated outside of acc_core
  typedef struct packed {
    logic lifecycle_escalation;
    logic illegal_bus_access;
    logic bad_internal_state;
  ...
  typedef enum logic [StateControllerWidth-1:0] {
    AccStateHalt        = 5'b00100,
    AccStateRun         = 5'b01010,
    AccStateStall       = 5'b10011,
    AccStateLocked      = 5'b11101
  } acc_state_e;
  ...
  typedef enum logic [StateStartStopWidth-1:0] {
    AccStartStopStateInitial             = 7'b1010011,
    AccStartStopStateHalt                = 7'b1111001,
    AccStartStopStateUrndRefresh         = 7'b0000110,
    AccStartStopStateRunning             = 7'b1001000,
    AccStartStopSecureWipeWdrUrnd        = 7'b0101100,
    AccStartStopSecureWipeAccModBaseUrnd = 7'b0010000,
    AccStartStopSecureWipeAllZero        = 7'b0110101,
    AccStartStopSecureWipeComplete       = 7'b0001011,
    AccStartStopStateLocked              = 7'b1101111
  } acc_start_stop_state_e;
...
typedef enum logic [StateScrambleCtrlWidth-1:0] {
  ...
  parameter otp_ctrl_pkg::acc_key_t RndCnstAccKeyDefault =
      128'h14e8cecae3040d5e12286bb3cc113298;
  parameter otp_ctrl_pkg::acc_nonce_t RndCnstAccNonceDefault =
      64'hf79780bc735f3843;

  typedef logic [63:0] acc_dmem_nonce_t;
  typedef logic [63:0] acc_imem_nonce_t;
endpackage
