 ...
 * Buffers used by disjoint operations share storage via aliasing. */
...
/* Give the masked ExpandS call stack headroom (recent gadget changes pushed it
 * past the old 224 B, spilling into matmul_nonce below); keygen_tmp moves up to
 * match. */
...
/* Masked keygen ExpandS scratch is fully overlaid (no storage here):
 * eta_scratch/eta_b2a reuse the unwritten sk-t0/pk via the kernel trick, and
 * eta_out/matmul_nonce/keygen_mask_stack alias the keygen-dead sig (above). */
...
/* c_poly (NTT(c) for masked sign + verify) overlays the BIG_BUF tail; the .set
 * is emitted after w1_polyvec below (it must be defined first). */

/* rhoprime/ctilde (sign/verify) alias zeta_shares; w1_repvec (verify) aliases
 * sk; the masked-sign scratch (sign_y/sign_tmp/sign_w1_repvec/sign_gamma1_buf)
 * aliases the sign-dead pk; mask_stack aliases tmp_poly (all .set above). */

/* Big polyvec buffer shared by disjoint ops: keygen t (2*K*1024), masked sign
 * w0_polyvec_shares (matrix-mult accumulator + packed b' + overlays), verify
 * w1_polyvec. Worst case 2*8*1024 = 16384. */
.balign 32
.globl t_polyvec
 ...
 * dead region past the packed-b'/c_poly_shares rows covers the same top slot.
 * Matches the per-op sign overlay (mldsa_sign_bss.s), just shifted to clear z. */
.globl c_poly
.globl y_poly
.globl s1_poly
 ...
 * to avoid colliding with the unprotected verify's c_poly). */
.globl sign_hint_b2a
