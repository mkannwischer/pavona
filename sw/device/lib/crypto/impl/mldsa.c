...
otcrypto_status_t otcrypto_mldsa44_sign_derand(
  ...
  if (launder32(sign_mode) == kOtcryptoMldsaSignModeMldsa) {
    ...
    result = mldsa44_signature_internal(
        (uint8_t *)signature.data, &signature_len, message.data, message.len,
        pre, 2 + context.len, (const uint8_t *)rnd.data,
        (const uint8_t *)secret_key->keyblob, 0, &ctx);
  } else if (launder32(sign_mode) == kOtcryptoMldsaSignModeExternalMu) {
    HARDENED_CHECK_EQ(sign_mode, kOtcryptoMldsaSignModeExternalMu);
    result = mldsa44_signature_internal(
        (uint8_t *)signature.data, &signature_len, message.data, kMldsaMuBytes,
        NULL, 0, (const uint8_t *)rnd.data,
        (const uint8_t *)secret_key->keyblob, 1, &ctx);
  } else {
    HARDENED_CHECK_NE(sign_mode, kOtcryptoMldsaSignModeMldsa);
    HARDENED_CHECK_NE(sign_mode, kOtcryptoMldsaSignModeExternalMu);
    result = mldsa44_signature_pre_hash_internal(
        (uint8_t *)signature.data, &signature_len, (uint8_t *)ph, ph_len,
        context.data, context.len, (const uint8_t *)rnd.data,
...
otcrypto_status_t otcrypto_mldsa44_verify(
  ...
  if (launder32(sign_mode) == kOtcryptoMldsaSignModeMldsa) {
    HARDENED_CHECK_EQ(sign_mode, kOtcryptoMldsaSignModeMldsa);
    result =
        mldsa44_verify((const uint8_t *)signature.data, signature.len,
                       message.data, message.len, context.data, context.len,
                       (const uint8_t *)public_key->key, &ctx);
  } else if (launder32(sign_mode) == kOtcryptoMldsaSignModeExternalMu) {
    HARDENED_CHECK_EQ(sign_mode, kOtcryptoMldsaSignModeExternalMu);
    result = mldsa44_verify_extmu((const uint8_t *)signature.data,
                                  signature.len, message.data,
                                  (const uint8_t *)public_key->key, &ctx);
  } else {
    HARDENED_CHECK_NE(sign_mode, kOtcryptoMldsaSignModeMldsa);
    HARDENED_CHECK_NE(sign_mode, kOtcryptoMldsaSignModeExternalMu);
    result = mldsa44_verify_pre_hash_internal(
        (const uint8_t *)signature.data, signature.len, (uint8_t *)ph, ph_len,
        context.data, context.len, (const uint8_t *)public_key->key,
        mldsa_native_prehash_alg(sign_mode), &ctx);
  }
...
otcrypto_status_t otcrypto_mldsa65_verify(
  ...
  if (launder32(sign_mode) == kOtcryptoMldsaSignModeMldsa) {
    HARDENED_CHECK_EQ(sign_mode, kOtcryptoMldsaSignModeMldsa);
    result =
        mldsa65_verify((const uint8_t *)signature.data, signature.len,
                       message.data, message.len, context.data, context.len,
                       (const uint8_t *)public_key->key, &ctx);
  } else if (launder32(sign_mode) == kOtcryptoMldsaSignModeExternalMu) {
    HARDENED_CHECK_EQ(sign_mode, kOtcryptoMldsaSignModeExternalMu);
    result = mldsa65_verify_extmu((const uint8_t *)signature.data,
                                  signature.len, message.data,
                                  (const uint8_t *)public_key->key, &ctx);
  } else {
    HARDENED_CHECK_NE(sign_mode, kOtcryptoMldsaSignModeMldsa);
...
otcrypto_status_t otcrypto_mldsa87_verify(
  ...
  if (launder32(sign_mode) == kOtcryptoMldsaSignModeMldsa) {
    HARDENED_CHECK_EQ(sign_mode, kOtcryptoMldsaSignModeMldsa);
    result =
        mldsa87_verify((const uint8_t *)signature.data, signature.len,
                       message.data, message.len, context.data, context.len,
                       (const uint8_t *)public_key->key, &ctx);
  } else if (launder32(sign_mode) == kOtcryptoMldsaSignModeExternalMu) {
    HARDENED_CHECK_EQ(sign_mode, kOtcryptoMldsaSignModeExternalMu);
    result = mldsa87_verify_extmu((const uint8_t *)signature.data,
                                  signature.len, message.data,
                                  (const uint8_t *)public_key->key, &ctx);
  } else {
    HARDENED_CHECK_NE(sign_mode, kOtcryptoMldsaSignModeMldsa);
