...
class ResultsTracker(object):
    ...
    def total_count(self):
        return (
            len(self.passing_paths)
            + len(self.failing_paths)
    ...
    def any_failed(self):
        return len(self.failing_paths) > 0

    def display_nicely(self):
        headers = ["Results:", "Files"]
        results = [
            ["Passed", len(self.passing_paths)],
            ["Failed", len(self.failing_paths)],
...
def check_paths(config, git_paths):
    ...
    try:
        all_matchers = {
            key: [LicenceMatcher(style, text, match_regex)
                  for text, match_regex in config.licences]
...
def check_file_for_licence(all_matchers, results, filepath):
    matcher_groups = detect_comment_char(all_matchers, filepath.name)
...
def main():
    desc = "A tool to check for a licence header in each source file"
    ...
    config.licences = []
    for licence in parsed_config.get("licences", []):
        text = LicenceHeader(licence["licence"])
        match_regex = licence.get("match_regex", "false")
        ...
        match_regex = (match_regex == "true")
        config.licences.append((text, match_regex))
